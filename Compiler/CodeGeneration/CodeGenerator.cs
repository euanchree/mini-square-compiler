    ...
    public class CodeGenerator
    {
        ...
        private TargetCode GenerateCodeFor(IAbstractSyntaxTreeNode node)
        {
            if (node is null)
            {
                ...
                WriteError("Tried to generate code for a null tree node");
                return null;
            }
            else if (node is ErrorNode)
            {
                ...
                WriteError("Tried to generate code for an error tree node");
                return null;
            }
            else
            {
                ...
                if (function == null)
                {
                    ...
                    WriteError($"Couldn't find the function {functionName} when generating code");
                    return null;
                }
                else
        ...
        /// <summary>
        /// Generates code for a quick if command
        /// </summary>
        ...
        private void GenerateCodeForLoopCommand(LoopCommandNode loopCommand)
        {
            ...
            Address jumpAddress2 = code.NextAddress;
            GenerateCodeFor(loopCommand.LoopCommand);
            Address jumpAddress = code.NextAddress;
            code.AddInstruction(OpCode.JUMP, Register.CB, 0, 0);
            Address loopAddress = code.NextAddress;
            GenerateCodeFor(loopCommand.WhileCommand);
            code.PatchInstructionToJumpHere(jumpAddress);
        ...
        private void GenerateCodeForVarParameter(VarParameterNode varParameter)
        {
            ...
            if (varParameter.Identifier.Declaration is IVariableDeclarationNode varDeclaration)
                ...
                //WriteError($" {varParameter.Position.ToString()} the identifier is not a variable and you should have picked this problem up during type checking.");
        }
        ...
        /// <summary>
        /// Write error message from the code generator
        /// </summary>
